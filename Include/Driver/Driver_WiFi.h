...
#define WIFI_API_VERSION      DRIVER_VERSION_MAJOR_MINOR(1,1)  /* API version */


#define _Driver_WiFi_(n)      Driver_WiFi##n
#define  Driver_WiFi_(n)     _Driver_WiFi_(n)


/****** WiFi SetOption/GetOption Function Option Codes *****/
#define WIFI_BSSID                      1U          ///< Station/AP Set/Get BSSID of AP to connect or of AP;        data = &bssid,    len =  6, uint8_t[6]
#define WIFI_TX_POWER                   2U          ///< Station/AP Set/Get transmit power;                         data = &power,    len =  4, uint32_t: 0 .. 20 [dBm]
#define WIFI_LP_TIMER                   3U          ///< Station    Set/Get low-power deep-sleep time;              data = &time,     len =  4, uint32_t [seconds]: 0 = disable (default)
#define WIFI_DTIM                       4U          ///< Station/AP Set/Get DTIM interval;                          data = &dtim,     len =  4, uint32_t [beacons]
#define WIFI_BEACON                     5U          ///<         AP Set/Get beacon interval;                        data = &interval, len =  4, uint32_t [ms]
#define WIFI_MAC                        6U          ///< Station/AP Set/Get MAC;                                    data = &mac,      len =  6, uint8_t[6]
#define WIFI_IP                         7U          ///< Station/AP Set/Get IPv4 static/assigned address;           data = &ip,       len =  4, uint8_t[4]
#define WIFI_IP_SUBNET_MASK             8U          ///< Station/AP Set/Get IPv4 subnet mask;                       data = &mask,     len =  4, uint8_t[4]
#define WIFI_IP_GATEWAY                 9U          ///< Station/AP Set/Get IPv4 gateway address;                   data = &ip,       len =  4, uint8_t[4]
#define WIFI_IP_DNS1                    10U         ///< Station/AP Set/Get IPv4 primary   DNS address;             data = &ip,       len =  4, uint8_t[4]
#define WIFI_IP_DNS2                    11U         ///< Station/AP Set/Get IPv4 secondary DNS address;             data = &ip,       len =  4, uint8_t[4]
#define WIFI_IP_DHCP                    12U         ///< Station/AP Set/Get IPv4 DHCP client/server enable/disable; data = &dhcp,     len =  4, uint32_t: 0 = disable, non-zero = enable (default)
#define WIFI_IP_DHCP_POOL_BEGIN         13U         ///<         AP Set/Get IPv4 DHCP pool begin address;           data = &ip,       len =  4, uint8_t[4]
#define WIFI_IP_DHCP_POOL_END           14U         ///<         AP Set/Get IPv4 DHCP pool end address;             data = &ip,       len =  4, uint8_t[4]
#define WIFI_IP_DHCP_LEASE_TIME         15U         ///<         AP Set/Get IPv4 DHCP lease time;                   data = &time,     len =  4, uint32_t [seconds]
#define WIFI_IP6_GLOBAL                 16U         ///< Station/AP Set/Get IPv6 global address;                    data = &ip6,      len = 16, uint8_t[16]
#define WIFI_IP6_LINK_LOCAL             17U         ///< Station/AP Set/Get IPv6 link local address;                data = &ip6,      len = 16, uint8_t[16]
#define WIFI_IP6_SUBNET_PREFIX_LEN      18U         ///< Station/AP Set/Get IPv6 subnet prefix length;              data = &len,      len =  4, uint32_t: 1 .. 127
#define WIFI_IP6_GATEWAY                19U         ///< Station/AP Set/Get IPv6 gateway address;                   data = &ip6,      len = 16, uint8_t[16]
#define WIFI_IP6_DNS1                   20U         ///< Station/AP Set/Get IPv6 primary   DNS address;             data = &ip6,      len = 16, uint8_t[16]
#define WIFI_IP6_DNS2                   21U         ///< Station/AP Set/Get IPv6 secondary DNS address;             data = &ip6,      len = 16, uint8_t[16]
#define WIFI_IP6_DHCP_MODE              22U         ///< Station/AP Set/Get IPv6 DHCPv6 client mode;                data = &mode,     len =  4, uint32_t: WIFI_IP6_DHCP_xxx (default Off)

/****** WiFi Security Type *****/
#define WIFI_SECURITY_OPEN              0U          ///< Open
#define WIFI_SECURITY_WEP               1U          ///< Wired Equivalent Privacy (WEP) with Pre-Sheared Key (PSK)
#define WIFI_SECURITY_WPA               2U          ///< WiFi Protected Access (WPA) with PSK
#define WIFI_SECURITY_WPA2              3U          ///< WiFi Protected Access II (WPA2) with PSK
#define WIFI_SECURITY_UNKNOWN           255U        ///< Unknown

/****** WiFi Protected Setup (WPS) Method *****/
#define WIFI_WPS_METHOD_NONE            0U          ///< Not used
#define WIFI_WPS_METHOD_PBC             1U          ///< Push Button Configuration
#define WIFI_WPS_METHOD_PIN             2U          ///< PIN

/****** WiFi IPv6 Dynamic Host Configuration Protocol (DHCP) Mode *****/
#define WIFI_IP6_DHCP_OFF               0U          ///< Static Host Configuration (default)
#define WIFI_IP6_DHCP_STATELESS         1U          ///< Dynamic Host Configuration stateless DHCPv6
#define WIFI_IP6_DHCP_STATEFULL         2U          ///< Dynamic Host Configuration statefull DHCPv6

/****** WiFi Event *****/
#define WIFI_EVENT_AP_CONNECT          (1UL << 0)   ///< Access Point: Station has connected;           arg = &mac, mac (uint8_t[6])
#define WIFI_EVENT_AP_DISCONNECT       (1UL << 1)   ///< Access Point: Station has disconnected;        arg = &mac, mac (uint8_t[6])
#define WIFI_EVENT_ETH_RX_FRAME        (1UL << 4)   ///< Ethernet Frame Received (in bypass mode only); arg = interface (0 = Station, 1 = Access Point)
...
typedef struct WIFI_CONFIG_s {
  ...
  const char   *pass;                               ///< Pointer to Password null-terminated string
        uint8_t security;                           ///< Security type (WIFI_SECURITY_xxx)
        uint8_t ch;                                 ///< WiFi Channel (0 = auto, otherwise = exact channel)
        uint8_t reserved;                           ///< Reserved
        uint8_t wps_method;                         ///< WiFi Protected Setup (WPS) method (WIFI_WPS_METHOD_xxx)
...
*/
typedef struct WIFI_SCAN_INFO_s {
...
*/
typedef struct WIFI_NET_INFO_s {
  char    ssid[32+1];                               ///< Service Set Identifier (SSID) null-terminated string
...
#define SOCKET_SOCK_STREAM              1           ///< Stream socket
#define SOCKET_SOCK_DGRAM               2           ///< Datagram socket

/****** Socket Protocol definitions *****/
#define SOCKET_IPPROTO_TCP              1           ///< TCP
#define SOCKET_IPPROTO_UDP              2           ///< UDP

/****** Socket Option definitions *****/
#define SOCKET_IO_FIONBIO               1           ///< Non-blocking I/O (Set only, default = 0); opt_val = &nbio, opt_len = sizeof(nbio), nbio (integer): 0=blocking, non-blocking otherwise
#define SOCKET_SO_RCVTIMEO              2           ///< Receive timeout in ms (default = 0); opt_val = &timeout, opt_len = sizeof(timeout)
#define SOCKET_SO_SNDTIMEO              3           ///< Send timeout in ms (default = 0); opt_val = &timeout, opt_len = sizeof(timeout)
#define SOCKET_SO_KEEPALIVE             4           ///< Keep-alive messages (default = 0); opt_val = &keepalive, opt_len = sizeof(keepalive), keepalive (integer): 0=disabled, enabled otherwise
#define SOCKET_SO_TYPE                  5           ///< Socket Type (Get only); opt_val = &socket_type, opt_len = sizeof(socket_type), socket_type (integer): SOCKET_SOCK_xxx
...
/**
  \fn            DRIVER_VERSION WIFI_GetVersion (void)
  \brief         Get driver version.
  \return        \ref DRIVER_VERSION
*/
/**
  \fn            WIFI_CAPABILITIES WIFI_GetCapabilities (void)
  \brief         Get driver capabilities.
  \return        \ref WIFI_CAPABILITIES
*/
/**
  \fn            int32_t WIFI_Initialize (WIFI_SignalEvent_t cb_event)
  \brief         Initialize WiFi Module.
  \param[in]     cb_event Pointer to \ref WIFI_SignalEvent_t
  \return        execution status
                   - \ref DRIVER_OK                : Operation successful
                   - \ref DRIVER_ERROR             : Operation failed
*/
/**
...
*/
/**
  \fn            int32_t WIFI_PowerControl (POWER_STATE state)
  \brief         Control WiFi Module Power.
  \param[in]     state     Power state
                   - \ref POWER_OFF                : Power off: no operation possible
                   - \ref POWER_LOW                : Low-power mode: sleep or deep-sleep depending on \ref WIFI_LP_TIMER option set
                   - \ref POWER_FULL               : Power on: full operation at maximum performance
  \return        execution status
                   - \ref DRIVER_OK                : Operation successful
                   - \ref DRIVER_ERROR             : Operation failed
                   - \ref DRIVER_ERROR_UNSUPPORTED : Operation not supported
                   - \ref DRIVER_ERROR_PARAMETER   : Parameter error (invalid state)
*/
/**
  ...
  \return        execution status
                   ...
                   - \ref DRIVER_ERROR             : Operation failed
                   - \ref DRIVER_ERROR_UNSUPPORTED : Operation not supported
                   - \ref DRIVER_ERROR_PARAMETER   : Parameter error (NULL module_info pointer or max_len equals to 0)
*/
/**
  ...
  \return        execution status
                   ...
                   - \ref DRIVER_ERROR             : Operation failed
                   - \ref DRIVER_ERROR_UNSUPPORTED : Operation not supported
                   - \ref DRIVER_ERROR_PARAMETER   : Parameter error (invalid interface, NULL data pointer or len less than option specifies)
*/
/**
  ...
  \return        execution status
                   ...
                   - \ref DRIVER_ERROR             : Operation failed
                   - \ref DRIVER_ERROR_UNSUPPORTED : Operation not supported
                   - \ref DRIVER_ERROR_PARAMETER   : Parameter error (invalid interface, NULL data or len pointer, or *len less than option specifies)
*/
/**
  \fn            int32_t WIFI_Scan (WIFI_SCAN_INFO_t scan_info[], uint32_t max_num)
  \brief         Scan for available networks in range.
  \param[out]    scan_info Pointer to array of WIFI_SCAN_INFO_t structures where available Scan Information will be returned
  \param[in]     max_num   Maximum number of Network Information structures to return
  \return        number of WIFI_SCAN_INFO_t structures returned or error code
                   - value >= 0                        : Number of WIFI_SCAN_INFO_t structures returned
                   - \ref DRIVER_ERROR             : Operation failed
                   - \ref DRIVER_ERROR_PARAMETER   : Parameter error (NULL scan_info pointer or max_num equal to 0)
*/
/**
  \fn            int32_t WIFI_Activate (uint32_t interface, WIFI_CONFIG_t *config)
  \brief         Activate interface (Connect to a wireless network or activate an access point).
  \param[in]     interface Interface (0 = Station, 1 = Access Point)
  \param[in]     config    Pointer to WIFI_CONFIG_t structure where Configuration parameters are located
  \return        execution status
                   - \ref DRIVER_OK                : Operation successful
                   - \ref DRIVER_ERROR             : Operation failed
                   - \ref DRIVER_ERROR_TIMEOUT     : Timeout occurred
                   - \ref DRIVER_ERROR_UNSUPPORTED : Operation not supported (security type, channel autodetect or WPS not supported)
                   - \ref DRIVER_ERROR_PARAMETER   : Parameter error (invalid interface, NULL config pointer or invalid configuration)
*/
/**
  ...
  \return        execution status
                   - \ref DRIVER_OK                : Operation successful
                   - \ref DRIVER_ERROR             : Operation failed
                   - \ref DRIVER_ERROR_PARAMETER   : Parameter error (invalid interface)
*/
/**
...
*/
/**
  \fn            int32_t WIFI_GetNetInfo (WIFI_NET_INFO_t *net_info)
  \brief         Get station Network Information.
  \param[out]    net_info  Pointer to WIFI_NET_INFO_t structure where station Network Information will be returned
  \return        execution status
                   - \ref DRIVER_OK                : Operation successful
                   - \ref DRIVER_ERROR             : Operation failed (station not connected)
                   - \ref DRIVER_ERROR_UNSUPPORTED : Operation not supported
                   - \ref DRIVER_ERROR_PARAMETER   : Parameter error (invalid interface or NULL net_info pointer)
*/
/**
  ...
  \return        execution status
                   ...
                   - \ref DRIVER_ERROR             : Operation failed
                   - \ref DRIVER_ERROR_UNSUPPORTED : Operation not supported
                   - \ref DRIVER_ERROR_PARAMETER   : Parameter error (invalid interface or mode)
*/
/**
  ...
  \return        execution status
                   - \ref DRIVER_OK                : Operation successful
                   - \ref DRIVER_ERROR             : Operation failed
                   - \ref DRIVER_ERROR_BUSY        : Driver is busy
                   - \ref DRIVER_ERROR_UNSUPPORTED : Operation not supported
                   - \ref DRIVER_ERROR_PARAMETER   : Parameter error (invalid interface or NULL frame pointer)
*/
/**
...
/**
  ...
  \return        status information
                   ...
                   - \ref SOCKET_EINVAL            : Invalid argument
                   - \ref SOCKET_ENOTSUP           : Operation not supported
                   - \ref SOCKET_ENOMEM            : Not enough memory
                   - \ref SOCKET_ERROR             : Unspecified error
*/
/**
  ...
  \return        status information
                   ...
                   - \ref SOCKET_ESOCK             : Invalid socket
                   - \ref SOCKET_EINVAL            : Invalid argument (address or socket already bound)
                   - \ref SOCKET_EADDRINUSE        : Address already in use
                   - \ref SOCKET_ERROR             : Unspecified error
*/
/**
  ...
  \return        status information
                   - 0                                 : Operation successful
                   - \ref SOCKET_ESOCK             : Invalid socket
                   - \ref SOCKET_EINVAL            : Invalid argument (socket not bound)
                   - \ref SOCKET_ENOTSUP           : Operation not supported
                   - \ref SOCKET_EISCONN           : Socket is already connected
                   - \ref SOCKET_ERROR             : Unspecified error
*/
/**
  ...
  \return        status information
                   ...
                   - \ref SOCKET_ESOCK             : Invalid socket
                   - \ref SOCKET_EINVAL            : Invalid argument (socket not in listen mode)
                   - \ref SOCKET_ENOTSUP           : Operation not supported (socket type does not support accepting connections)
                   - \ref SOCKET_ECONNRESET        : Connection reset by the peer
                   - \ref SOCKET_ECONNABORTED      : Connection aborted locally
                   - \ref SOCKET_EAGAIN            : Operation would block or timed out (may be called again)
                   - \ref SOCKET_ERROR             : Unspecified error
*/
/**
  ...
  \return        status information
                   ...
                   - \ref SOCKET_ESOCK             : Invalid socket
                   - \ref SOCKET_EINVAL            : Invalid argument
                   - \ref SOCKET_EALREADY          : Connection already in progress
                   - \ref SOCKET_EINPROGRESS       : Operation in progress
                   - \ref SOCKET_EISCONN           : Socket is connected
                   - \ref SOCKET_ECONNREFUSED      : Connection rejected by the peer
                   - \ref SOCKET_ECONNABORTED      : Connection aborted locally
                   - \ref SOCKET_EADDRINUSE        : Address already in use
                   - \ref SOCKET_ETIMEDOUT         : Operation timed out
                   - \ref SOCKET_ERROR             : Unspecified error
*/
/**
  ...
  \return        status information
                   ...
                   - \ref SOCKET_ESOCK             : Invalid socket
                   - \ref SOCKET_EINVAL            : Invalid argument (pointer to buffer or length)
                   - \ref SOCKET_ENOTCONN          : Socket is not connected
                   - \ref SOCKET_ECONNRESET        : Connection reset by the peer
                   - \ref SOCKET_ECONNABORTED      : Connection aborted locally
                   - \ref SOCKET_EAGAIN            : Operation would block or timed out (may be called again)
                   - \ref SOCKET_ERROR             : Unspecified error
...
/**
  ...
  \return        status information
                   - 0                                 : Operation successful
                   - \ref SOCKET_ESOCK             : Invalid socket
                   - \ref SOCKET_EAGAIN            : Operation would block (may be called again)
                   - \ref SOCKET_ERROR             : Unspecified error
*/
/**
  ...
  \return        status information
                   ...
                   - \ref SOCKET_ENOTSUP           : Operation not supported
                   - \ref SOCKET_ETIMEDOUT         : Operation timed out
                   - \ref SOCKET_EHOSTNOTFOUND     : Host not found
                   - \ref SOCKET_ERROR             : Unspecified error
*/
/**
  ...
  \return        execution status
                   ...
                   - \ref DRIVER_ERROR_TIMEOUT     : Timeout occurred
                   - \ref DRIVER_ERROR_UNSUPPORTED : Operation not supported
                   - \ref DRIVER_ERROR_PARAMETER   : Parameter error (NULL ip pointer or ip_len different than 4 or 16)
*/
/**
  \fn            void WIFI_SignalEvent (uint32_t event, void *arg)
...
typedef void (*WIFI_SignalEvent_t) (uint32_t event, void *arg); ///< Pointer to \ref WIFI_SignalEvent : Signal WiFi Event.


/**
\brief WiFi Driver Capabilities.
*/
typedef struct _WIFI_CAPABILITIES {
...
*/
typedef struct _DRIVER_WIFI {
  DRIVER_VERSION    (*GetVersion)                  (void);
  WIFI_CAPABILITIES (*GetCapabilities)             (void);
  int32_t           (*Initialize)                  (WIFI_SignalEvent_t cb_event);
  ...
  int32_t           (*Scan)                        (WIFI_SCAN_INFO_t scan_info[], uint32_t max_num);
  int32_t           (*Activate)                    (uint32_t interface, const WIFI_CONFIG_t *config);
  int32_t           (*Deactivate)                  (uint32_t interface);
  uint32_t          (*IsConnected)                 (void);
  int32_t           (*GetNetInfo)                  (WIFI_NET_INFO_t *net_info);
