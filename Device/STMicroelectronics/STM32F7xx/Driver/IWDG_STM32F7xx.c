/*
 * Copyright (C) 2020-2023 Sergey Koshkin <koshkin.sergey@gmail.com>
 * All rights reserved.
 *
...
#include <Driver/IWDG_STM32F7xx.h>
...
int32_t IWDG_Initialize(const IWDG_Init_t *init)
{
  ...
  while (IWDG->SR != 0UL);
  /* If window parameter is different than current value */
  if (IWDG->WINR != init->Window) {
    /* Modify window register */
    IWDG->WINR = init->Window;
  }
  else {
