...
int32_t USART_Control(uint32_t control, uint32_t arg, const USART_RESOURCES *usart)
{
  ...
  switch (control & USART_CONTROL_Msk) {
    ...
    case USART_CONTROL_BREAK:
      if (arg) {
        if (usart->xfer->send_active != 0U) { return DRIVER_ERROR_BUSY; }
  ...
  switch (control & USART_CONTROL_Msk) {
    ...
    case USART_SET_IRDA_PULSE:
      if (usart->info->mode == USART_MODE_IRDA) {
        if (arg != 0U) {
          uint32_t i;
    ...
    case USART_SET_SMART_CARD_GUARD_TIME:
      if (usart->info->mode == USART_MODE_SMART_CARD) {
        if (arg > 255U)
          return DRIVER_ERROR;
...
static void USART_RX_DMA_Callback(uint32_t event, const void *param)
{
  ...
  if (event & DMA_EVENT_TRANSFER_COMPLETE) {
    ...
    if (info->mode == USART_MODE_SYNCHRONOUS_MASTER) {
      ...
      switch (val) {
        case USART_SYNC_MODE_TX:
          cb_event = USART_EVENT_SEND_COMPLETE;
          break;
        case USART_SYNC_MODE_RX:
          cb_event = USART_EVENT_RECEIVE_COMPLETE;
          break;
        case USART_SYNC_MODE_TX_RX:
          cb_event = USART_EVENT_TRANSFER_COMPLETE;
          break;
        default:
          break;
      }
