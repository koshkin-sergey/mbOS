 ...
 *
 * Project: SPI Driver for STMicroelectronics STM32F7xx
 */
...
#ifdef SPI_DMA_TX
static void SPIx_TX_DMA_Callback(uint32_t event, const void *param);
#endif

#ifdef SPI_DMA_RX
static void SPIx_RX_DMA_Callback(uint32_t event, const void *param);
#endif
...
int32_t SPI_Control(uint32_t control, uint32_t arg, SPI_RESOURCES *spi)
{
  ...
  /* Data Bits */
  val = (control & SPI_DATA_BITS_Msk) >> SPI_DATA_BITS_Pos;
  if ((val < 4U) || (val > 16)) {
